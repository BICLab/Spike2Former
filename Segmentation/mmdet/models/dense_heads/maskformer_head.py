...
from mmdet.models.layers.transformer.utils import QueryProposal
from mmdet.models.layers.pixel_decoder import PixelDecoder
from mmdet.models.layers.transformer.mmcv_spike.SNN_core import MLP
...
class MaskFormerHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.decoder_post_norm = nn.Sigmoid()
        # 这个spike影响mask的分割质量，在D=1时还需要保持4量化，否则不work
        self.decoder_out_spike = Q_IFNode(surrogate_function=Quant())
        self.cls_embed = nn.Linear(feat_channels, self.num_classes + 1)

        self.mask_embed_spike = Q_IFNode(surrogate_function=Quant())
        # self.mask_embed = nn.Sequential(
        #     nn.Linear(feat_channels, feat_channels, bias=False), Q_IFNode(surrogate_function=Quant()),
        #     nn.Linear(feat_channels, feat_channels, bias=False), Q_IFNode(surrogate_function=Quant()),
        #     nn.Linear(feat_channels, out_channels))
        # This for the Q-IFNode MLP.
        self.mask_embed = MLP(in_dim=feat_channels, out_dim=out_channels, layer=3, T=self.T, quant_const=self.alpha)

        self.w = nn.Parameter(torch.ones(1))
        self.shortcut_conv_spike =Q_IFNode(surrogate_function=Quant())
        self.shortcut_conv = nn.Sequential(
            nn.Conv1d(self.num_queries, self.num_queries, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm1d(self.num_queries))
    ...
    def init_weights(self) -> None:
        ...
        # nn.init.constant_(self.mask_embed[-1].bias, 0)
        # trunc_normal_(self.mask_embed[-1].weight, std=0.02)
    ...
    def forward(self, x: Tuple[Tensor],
        ...
        mask_features, memory, multi_scale_memorys = self.pixel_decoder(x, batch_img_metas)
        # shape = (num_queries, embed_dims)
        t = memory.shape[0]
        query_feat = self.query_feat.weight.unsqueeze(0).repeat((t, batch_size, 1, 1))
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            # shape (t, batch_size, c, h, w) -> (batch_size, h*w, c)
            decoder_input = decoder_input.flatten(3).permute(0, 1, 3, 2)
        ...
        out_dec = []
        out_dec.append(query_feat)  # Mask2former: Supervise the query intial
        # level index 00 11 22
        for i in range(self.num_transformer_decoder_layers):
            ...
            # import pdb; pdb.set_trace()
            out_dec.append(query_feat)

        out_dec = torch.stack(out_dec)  # [6, bs, nq, dim]
        ln, t, bs, nq, embed_dim = out_dec.shape
        
        ####### SDME #######
        out_dec = self.decoder_post_norm(out_dec)  # nn.Simoid
        out_dec_ = self.alpha * self.decoder_out_spike(out_dec)   # [6, 8, 100, 256]
        all_cls_scores = self.cls_embed(out_dec_).mean(1)  # initial bias with zero

        mask_embed = self.mask_embed(out_dec_) + \
                     self.w * (
                         self.shortcut_conv(self.alpha * self.shortcut_conv_spike(out_dec).reshape(ln * t * bs, nq, embed_dim))
                         .view(ln, t, bs, nq, embed_dim).contiguous())
        mask_embed = self.alpha * self.mask_embed_spike(mask_embed)  # post spike
        all_mask_preds = torch.einsum('ltbqc,tbchw->ltbqhw', mask_embed,
                                      mask_features).mean(1)  # [layer, bs, nq, embed_dim] * [bs, c, h, w]

        # mask_feature = self.Identity(all_mask_preds[-1])
