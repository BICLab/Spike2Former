...
class LoadImagesAndLabels(Dataset):
    ...
    def build_dataset(self, path, save_file):
        ...
        for file_name in files:
            ...
            for b in boxes_per_ts:
                ...
                scale = np.array([[1, 0, 0, 0, 0], [0, 1 / w, 0, 0, 0], [0, 0, 1 / h, 0, 0], [0, 0, 0, 1 / w, 0],
                                  [0, 0, 0, 0, 1 / h]], dtype=float)
    ...
    def create_targets(self, boxes):  # 解析包围盒和标签；只需要更改这个函数！！！！合并box和label(为什么会有负的？)
        torch_boxes = torch.from_numpy(
            structured_to_unstructured(boxes[['x', 'y', 'w', 'h']], dtype=np.float32))  # 抽出对应的4维
    ...
    def create_data(self, events, img=None):
        ...
        if len(events):
            for i in range(self.T):
                if len(events[i]):
                    assert events[i]['x'].max() < width, "out of bound events: x = {}, w = {}".format(
                        events[i]['x'].max(), width)
                    assert events[i]['y'].max() < height, "out of bound events: y = {}, h = {}".format(
                        events[i]['y'].max(), height)
                img[i, events[i]['x'], events[i]['y'], :] = 255 * events[i]['p'][:,
                                                                  None]  # img默认为灰度127，有正事件则为255，无正事件则为0
...
image_shape = (240, 304)
T = 4
path = '/public/liguoqi/lzx/data/ddd17_seg/data'  # use your path
outpath = '/public/liguoqi/lzx/data/ddd17_seg/DDD2017'


"""
    TODO:
    1.读取dat，xyp格式的文件,查看文件中的内容
    2.按照时间步拆分数据集/聚合数据集
    3.生成对应的标记文件
    4.按照ADE20k的格式存储
"""
...
for dir in DIR:
    import pdb;
    pdb.set_trace()
    video = PSEELoader(os.path.join(path, dir, 'events.dat.t'))
    with open(os.path.join(path, dir, 'events.dat.t'), 'rb') as file:
        data = file.read()
        # 假设 .dat.t 文件是由浮点数构成的
        float_data = struct.unpack('f' * (len(data) // 4), data)
    train = np.loadtxt(path+'/'+dir+'/events.dat.t', encoding='utf-8')
